...
export const acceptInvite = async (request: Request, h: ResponseToolkit) => {
  ...
  try {
    ...
    const invite = (await Invite.findOne({
      ...
      include: [{ model: School }, { model: Role }],
      transaction,
    })) as any;
    if (!invite) {
      await transaction.rollback();
    ...
    if (invite.expiresAt < new Date()) {
      await transaction.rollback();
      return error(null, "Invite has expired", statusCodes.BAD_REQUEST)(h);
    }
    if (!user.isTempPassword) {
      await transaction.rollback();
      return error(
        null,
        "Must log in with temporary password to accept invite",
        statusCodes.BAD_REQUEST
      )(h);
    }
    ...
    // Activate user
    user.isActive = true;
    user.isTempPassword = false; // Require password reset
    await user.save({ transaction });
    ...
    return success(
      { schoolId: invite.schoolId, roleId: invite.roleId },
      "Invite accepted successfully. Please reset your password.",
      statusCodes.SUCCESS
    )(h);
  } catch (err: any) {
    await transaction.rollback();
    return error(
      null,
      err.message || "Failed to accept invite",
      statusCodes.SERVER_ISSUE
    )(h);
  }
};

export const rejectInvite = async (request: Request, h: ResponseToolkit) => {
  const { token } = request.params;
  const user = request.auth.credentials as any;
  ...
  try {
    ...
    if (!user.isTempPassword) {
      ...
      return error(
        null,
        "Must log in with temporary password to reject invite",
        statusCodes.BAD_REQUEST
      )(h);
    }
    ...
    // Deactivate user if created for this invite
    if (!user.isActive) {
      user.isActive = false;
      await user.save({ transaction });
    }
    ...
    return error(
      null,
      err.message || "Failed to reject invite",
      statusCodes.SERVER_ISSUE
    )(h);
  }
};
